...
psm <- MSnbase::combine(psml[[1]], psml[[2]])
for (i in 3:length(psml)) {
  psm <- MSnbase::combine(e, psml[[i]])
}
...
length(a_p[,1])
a_p[[2]][500]
p <- as.list(a_p$pepSeq)
names(p) <- a_p$accession
p
p[500:502]
length(p)

p <- sapply(p, function(x) strsplit(x,","))
class(p)
p[500:502]
pu <- sapply(p, function(x) unique(x))
class(pu)
pu[500:502]

p_count <- sapply(p, function(x) length(unique(x)))
p_count[500:502]

length(p_count[p_count == 1])
